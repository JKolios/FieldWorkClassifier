...
func indexDoc(c *gin.Context) {
	...
	defaultIndex := c.MustGet("Config").(*config.Settings).DefaultIndex
	...
	resp, err = es.IndexDocMapping(client, incoming.Index, incoming.Type, incoming.Body)
...
func getDoc(c *gin.Context) {
	...
	esResp, err := es.GetDoc(client, requestedIndex, requestedType, requestedID)
...
func termQuery(c *gin.Context) {
	...
	client := c.MustGet("ESClient").(*elastic.Client)
	requestPayload := &termSearchPayload{From: 0, MaxHits: 10, Index: c.MustGet("Config").(*config.Settings).DefaultIndex}
